...
PR #462 で本導線を実装し、未接続だったフォーム自動mount/ACK、受入条件のfield競合、
JU調査UI、追加contextの描画、依存更新時のProposal再確認、保存receipt付きGap復帰を修正した。
保存後に新しく編集されたdraftは遅延した成功応答で閉じない。receipt参照だけをURLに残し、
再読み込み後も保存された版へ到達できる。未保存本文はURLや新規永続領域へ複製しない。
JUの調査対象を絞る任意入力は既存investigationのresearch_focusへ渡す。候補が見つからない結果を
成功扱いせず、画面内に停止理由を示す。昇格しただけでは調査実行済みと表示しない。

[監査記録](decision-discussion-audit-2026-09-12.md) は自動テスト、実API/実LLM、ブラウザ操作と
未実測を区別する。代表利用者・実スクリーンリーダー環境などの決定と残る人間の利用評価は
[#463](https://github.com/dx-junkyard/probe-agent/issues/463) に移管した。DD-AC-04/05 の実利用評価が
全件完了したという意味で元Issueを閉じるものではない。
