...
- **2 段階の状態決定**(#343 §2.2): 第 1 段は #287 の Alignment 分類ルール表と
  同じ first-match(12 行)で候補状態を決め、第 2 段で「完了済み状態への後退は
  明示確認」を適用する。ルール表だけでは基準 C を実現できない — 回答修正で
  未回答質問が発生した瞬間、候補状態は無条件に `W3` になる。第 2 段は
  `reached_state`(到達した最も進んだ状態)と戻り要求ごとの承諾を永続事実
...
- **承認セットと差分の対応**(#343 §2.2 行 10): `W5` の条件に「承認済み提案が
...
- **C: `reached_state` と戻り要求**(§2.2 第 2 段)。システムが機械的に導出・
  記録する進行の事実。
