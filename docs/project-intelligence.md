...
- 5 カテゴリ(Vision / System purpose / Capabilities / API boundaries /
  ...
  「未設定」として扱えるようにしている。
- カテゴリ状態は `confirmed` / `review` / `missing` の 3 値のみ。内容が
...
- 「未解決の質問が無い」ことを根拠にした `confirmed` を出さず、4 値目の
  `unknown`(判定できません)にする。内容の有無(`missing`)と gap の有無
  (`review`)は session 詳細だけで判定できるので、そちらはそのまま出す
- Q&A 進捗カードを再試行付きのエラー表示 / 読み込み表示にし、未解決一覧には
  「Q&A 由来の項目が含まれていない」ことを明示する

`unknown` は issue §3 の 3 値を増やしたのではなく、「取得できていない」を
確認済みと言い切らないための判定不能値で、Q&A が読めている限り現れない
(#341 の データ不足 / 取得失敗 と同じ考え方)。
