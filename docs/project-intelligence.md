...
- **サンプル誤り発見時の分類ルール再評価(§5.4 後半 → #310)**: 完了。
  ...
  `reason_code` とポリシー来歴を `alignment_rule_objection` に不変の監査
  事実として記録する。System 単位の API はルール別に異議数を決定的に集計し、
  人間が明示して同じ reason_code の現行 `no_review_required` 項目を再確認
  対象へ戻せる。再確認化は決定的分類を変更せず、通常の手動回答/修正だけが
  対象を解消する（`decision_method: manual` を維持）。
