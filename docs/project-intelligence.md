...
- 未解決事項は `open_questions` と未解決 Q&A を突き合わせ、`qa_id`(無ければ
  質問文)で重複排除してから優先度順に並べる。一覧と Q&A 合計から外すのは
...
重複排除、`skipped` が未解決として数えられカテゴリを要確認にすること、
Q&A 3 区分の合計一致、修正手段の可否と理由、質問単位の移動先候補、カード
選択とキーボード操作、CTA のフォーカス移動を検証する。画面全体の組み上がり
(セッション一覧 / 詳細それぞれの API エラー表示と再試行、選んだ質問への
フォーカス移動を含む)は `dashboard-contracts.test.tsx` の Interview page 群。
