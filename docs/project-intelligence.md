...
「変更後の再確認待ち」は `change_count`(セクション差分の件数)と
`recheck_claim_count`(claim_digest が変わった主張の件数)の **両方** を見る。
両者は測っているものが違う — セクション差分は名前・説明・確信度しか見ないが、
`claim_digest` は根拠・関連ドキュメント・子要素も含む。根拠だけが差し替わった
確認済みの主張は前者に現れず、片方だけを見ていると「進行可能」として通過する。
