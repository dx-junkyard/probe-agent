...
- 未解決事項は `open_questions` と未解決 Q&A を突き合わせ、`qa_id`(無ければ
  質問文)で重複排除してから優先度順に並べる。`answered` は解決済み、
  `revised` は後続行に置き換えられた履歴、`skipped` は見送り確定なので
  一覧からも Q&A 合計からも外す。合計 = 回答済み + 確認待ち + 未回答 が
  常に成り立つ。
...
根拠表示の処理を重複実装しない。実行できない手段は **理由付きの disabled**
...
重複排除、Q&A 3 区分の合計一致、修正手段の可否と理由、カード選択と
キーボード操作、CTA のフォーカス移動を検証する。画面全体の組み上がりは
`dashboard-contracts.test.tsx` の Interview page 群。
