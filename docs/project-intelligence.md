...
- **2 段階の状態決定**(#343 §2.2): 第 1 段は #287 の Alignment 分類ルール表と
  同じ first-match(9 行)で候補状態を決め、第 2 段で「完了済み状態への後退は
...
- **情報役割 `R1`-`R6`**(#344): 現在地 / 必須作業 / 根拠 / 詳細 /
  ...
  主要 69 要素を棚卸しし、状態 × 役割のマトリクスに割り当てた。
...
- **例外 `E1`-`E14`**(#346): ブロッキング / 劣化 / 情報 の 3 区分。区分は
  処理の種類ではなく **失敗時点で有効な結果が残っているか** で決まる。
  したがって突き合わせ失敗は `E4-a`(初回失敗、有効な項目が 0 件 →
  ブロッキング。ズレを一度も確認しないまま `W5` へ進ませない)と
  `E4-b`(再構築失敗、前回の項目が残る → 劣化)に分かれる。
...
- **C: `reached_state` と戻り要求ごとの承諾**(§2.2 第 2 段)。これが無いと
  「完了済み状態への後退は明示確認」が成立しない。

A・B・C は表示状態の決定にのみ用い、人間のゲートを増やしも減らしもしない。
それ以外の状態(`W0-A` / `W0-B` / `W2`-`W5` / `W7`)は既存の永続事実から
決定的に導出できる。
